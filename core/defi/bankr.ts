...
export async function callBankrLLM(
  ...
  // Bankr uses OpenAI-compatible endpoint with wallet billing
  const resp = await fetch('https://api.bankr.bot/v1/chat/completions', {
    method: 'POST',
    headers: {
...
export async function getBankrWalletBalance(): Promise<{
  ...
  try {
    const resp = await fetch('https://api.bankr.bot/v1/wallet', {
